...
- Wenn wir über unsere Daten sprechen, verwenden wir bestimmte Wörter, um uns auf verschiedene Teile zu beziehen:
...
- eine Variable ist *numerisch* (d. h. quantitativ), wenn sie eine große Bandbreite an numerischen Werten annehmen kann
    ...
    + Alter, Größe, Reaktionszeiten, Formathäufigkeiten
...
- Die üblichen Variablen, die in einem Experiment zur lexikalischen Entscheidungsaufgabe erhoben werden, sind:
  ...
  + Wortkategorie (z. B. real/pseudo, Substantiv/Verb)
  + Worthäufigkeit
...
- Als nächstes teilen wir `ggplot()` mit, wie wir unsere Variablen visuell darstellen wollen
    - Wir verwenden das Argument `Mapping` von `ggplot()`, um zu definieren, wie die Variablen auf visuelle Eigenschaften (*Ästhetik*) *gemappt* werden
- Für unser erstes Diagramm wollen wir die Reaktionszeiten (RT) auf der X-Achse (dem unteren Teil des Diagramms) abbilden.
    - Wir verpacken die protokollierte "RT" in die Funktion "exp()", um die "RT" in Millisekunden zu erhalten (aus Gründen, die wir nicht diskutieren werden).
...
- Nehmen Sie die folgenden Änderungen vor, um unser Balkendiagramm darzustellen
  ...
  + Ersetzen Sie `geom_histogram()` durch `geom_bar()`
    + Entfernen Sie die Argumente für das Histogramm (nicht Position oder Alpha)
  + ändern Sie die Beschriftungen entsprechend
